...
public class OrderServiceImpl implements OrderService {
    ...
    public PurchaseOrder saveOrder(PurchaseOrder purchaseOrder) {
        if (purchaseOrder.getCustomer() == null || purchaseOrder.getCustomer().getId() <= 0) {
            throw new BadRequestException("The order should be associated with a customer id");
        }
        customerRepository.findById(purchaseOrder.getCustomer().getId())
                .orElseThrow(() -> new BadRequestException("The associated customer does not exist"));
        if (purchaseOrder.getLineItems() == null || purchaseOrder.getLineItems().size() == 0) {
            throw new BadRequestException("The order should have at least one product item");
        }
    ...
    @Override
    public PurchaseOrder updateOrder(PurchaseOrder purchaseOrder, long id) {
        ...
        if (purchaseOrder.getCustomer() != null && !Objects.equals(existingOrder.getCustomer().getId(), purchaseOrder.getCustomer().getId())) {
            throw new BadRequestException("The customer of the order cannot be changed");
        }
        customerRepository.findById(purchaseOrder.getCustomer().getId())
                .orElseThrow(() -> new BadRequestException("The associated customer does not exist"));
        if (purchaseOrder.getLineItems() == null || purchaseOrder.getLineItems().size() == 0) {
